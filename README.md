...
- **Day two**: `monup diff` shows drift between the output directory and
  the current plan (hand edits, stale files, pending changes);
  `monup watch` polls Docker and reports — or with `--auto-apply`,
  applies — plan changes as containers come and go.
